...
class Simulator:
        ...
        while not game_over:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if self.background_ctrl.fuzzy_button.collidepoint(event.pos):
                        self.background_ctrl.change_color_fuzzy_button()
                        self.isFuzzyOn = not self.isFuzzyOn
            ...
            if not self.is_extended:
                if current_green_light_remaining_time <= Config['simulator']['seconds_before_extension']:
                    fuzzy_score = self.calculate_fuzzy_score(self.moving_averages, self.isFuzzyOn)
    ...
    def calculate_fuzzy_score(self, moving_averages, isFuzzyOn):
        ...
        if traffic_state == DoubleLane.Vertical:
            return self.traffic_ctrl.calculate_fuzzy_score(moving_averages[Lane.top_to_bottom], moving_averages[Lane.left_to_right], ext_count, isFuzzyOn)
        elif traffic_state == DoubleLane.Horizontal:
            return self.traffic_ctrl.calculate_fuzzy_score(moving_averages[Lane.left_to_right], moving_averages[Lane.top_to_bottom], ext_count, isFuzzyOn)
